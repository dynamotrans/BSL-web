...
- **Responsive del panel en iPhone**: el botón "Ocupada" del calendario compartía la clase `occ` con el cuadro de Ocupación (`min-width:720px`) y ensanchaba toda la página. Renombrado a `.occgrid`. Además: casillas de fecha a prueba de Safari (`appearance:none`), rejillas con `minmax(0,1fr)` y ajustes para pantallas de 320 px. **Lección: prefijar las clases nuevas para no chocar (ya pasó con `.bar` y `.occ`), y al escanear desbordes no dar por bueno lo que recorta el `body`.**
- Botón flotante "volver arriba" (flecha) encima del de WhatsApp; aparece al bajar.
- Paginación (10 por página; inquilinas 12) en Cobros, Inquilinas, Incidencias y en los cobros/incidencias de cada ficha. En móvil los cobros se ven como tarjetas (concepto + importe, vence · inquilina · habitación, estado + botón).
- **Fichas reales de las 8 habitaciones** (`FICHAS` en `js/rooms.js`): Azahar, Jazmín, Dalia, Azucena, Tulipán, Malva, Girasol 330 €, Margarita, con m² con decimales y camas 105/110/140. Se aplican solas: la web pública las muestra si el almacén aún tiene nombres genéricos, y el panel las guarda en el almacén la primera vez que se entra (marca `ajustes.fichas = true`, no se repite). No tocan fotos, equipamiento, gastos ni fechas. Camas admitidas: 90/105/110/120/135/140/150.
- Fotos nuevas: `salon-patio.jpg`, `cocina-planta-baja.jpg`, `cocina-planta-baja-2.jpg`, `cocina-planta-alta.jpg`. "La vivienda" pasa a 4 fotos escalonadas (baños, habitaciones, cocinas, salón). Galería con 9 fotos. Textos: 2 cocinas (una en cada planta), 4 baños (dos en cada planta), salón con salida al patio.
- **Pantalla vacía en iPhone al hacer scroll/recargar**: Safari dejaba de pintar capas por exceso de efectos. Quitadas las animaciones infinitas (brillo difuminado girando en el precio, aro del WhatsApp, estrellas), el `backdrop-filter` de la barra y de las ventanas, y el desenfoque del logo; la foto de la portada va en capa estable (`will-change`, ampliación 1,7× en vez de 2,15×) y `.stage` con `contain:paint` y fondo. **Lección: en iOS evitar animaciones infinitas, `filter:blur` grandes y `backdrop-filter`.**
- Sección comparativa **"BSL o residencia · Vivir como en casa por menos de la mitad"** tras "Para las familias": 8 filas (precio 330 vs desde 800, ubicación, solo chicas vs mixtas, día a día andando, estudio/biblioteca 24 h, 2 cocinas vs comedor, espacio 205 m² + azoteas, trato cercano) con nota de precios orientativos.

## Cómo preparar cambios de datos (Claude no puede escribir en el almacén desde la sesión)
La red de la sesión bloquea `bsl-web.vercel.app`, así que los cambios de datos se preparan en **`api/_cambios.js`** (privado: solo se sirve con sesión por `api/cambios.js`, puede llevar datos personales). Al entrar al panel, la propietaria ve **"Cambios preparados por Claude"** con Aplicar / Descartar, uno a uno.
- Cada cambio: `{ id: 'AAAA-MM-DD-n' (único, nunca reutilizar), fecha, resumen, ops: [...] }`. Tipos de operación documentados en la cabecera de `api/_cambios.js` (habitacion, intervalo, inquilina, contrato, fin-contrato, cobro-pagado, incidencia, ajustes).
- Se aplica todo o nada (si una operación falla, se deshace y queda pendiente con el motivo). Lo aplicado/descartado queda registrado en `gestion.cambios`, así que los cambios ya resueltos pueden quedarse en el archivo o borrarse.
- Antes de preparar un cambio, leer el estado actual con `web_fetch_vercel_url` de `/api/datos` (público, sin datos personales). Los datos personales (`/api/gestion`) no se pueden leer desde la sesión.
- Calendario de cada habitación: el formulario de "añadir intervalo" pasa a **asistente por pasos** (1 ¿Ocupada o libre? → 2 ¿Qué fechas? curso completo / desde un día hasta fin de curso / otras → 3 fechas si hace falta → resumen en una frase + Guardar). Pulsar un mes del cuadro rellena "otras fechas" con ese mes.
